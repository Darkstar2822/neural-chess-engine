...
class ParallelSelfPlay:
    ...
    def generate_training_games(self, num_games: int = 100) -> Tuple[List[np.ndarray], List[np.ndarray], List[float]]:
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        states, policies, values = future.result(timeout=60)  # 60 second timeout per game
        ...
        print(f"🎯 Generated {len(all_states)} training examples from {completed_games} games")
